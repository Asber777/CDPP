...
class Robot(Dynamic):
    ...
    def trace(self):
        if self.reach_subgoal():
            self.current_goal_index -=1
            if self.current_goal_index==-1:#reach
                if self.reach_goal(self.getLocation()):
                    print("Reach Destination")
                    return True
                else:
                    raise Exception('trace到-1了但是没有到达终点')
                    return False
            else:
                print("robot reach its local goal%d"%(self.current_goal_index+1))
    ...
    def draw_path_and_plot(self,path_list,map_img):
        for robot_index, path in enumerate(path_list):
            if path!=None:
                ...
                for i in range(length-1):#only draw the path ,do not set pre-set pheronomone
                    cv2.line(map_img,path[i],path[i+1],(255,robot_index*10,int(255*i/length)),4)
        return draw_world(map_img,self.crowd_list,self.robot_list,show=True)
'''修改后的双目标优化'''
        ...
        for robot_index,path in enumerate(path_list):
            if path !=None:
                ...
                for i in range(length-1):#update pre-set pheronomone here
                    cv2.line(map_img,path[i],path[i+1],(255,robot_index*10,int(255*i/length)),int(10*(i+2)/length))
        draw_world(map_img,self.crowd_list,self.robot_list,show=True)
        return map_img
...
#--------------------------------------------------------------
def PRM_A_START_APF(mr,foldname,D_max=9*CIRCLE_R,PRM_POINT_NUM = 150,PRM_DISTANCE = 150):
    #一些状态的初始化以及环境的初始化
    collision_time,reward,step_num = 0,0,0
    mr.D_max = D_max
    mr.reset()
    sign,path_list,img = mr.prm_planning(p=False,show=True,s=PRM_POINT_NUM,n=PRM_DISTANCE)
    if sign:
